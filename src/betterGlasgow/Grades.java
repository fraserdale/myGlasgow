...
public class Grades {
	...
	public List<HashMap<String, String>> getGrades() throws IOException {
		...
		Document doc = Jsoup.connect(GRADES_URL).header("authorization", "Basic " + this.user.getCreds()).get();
	...
	public void prettify() {
		StringUtils.println("\u2500", 42);
		System.out.println(
				StringUtils.center("Course ID", 12) + " \u2502 " + StringUtils.center("Course Name", 18) + " \u2502 " + StringUtils.center("Grade", 3));
		StringUtils.println("\u2500", 42);
		for (HashMap<String, String> subject : this.grades) {
			System.out.println(String.format("%1$-12s",
					subject.get("courseID").substring(0, Math.min(subject.get("courseID").length(), 12)))
					+ " \u2502 "
					+ String.format("%1$-18s",subject.get("courseName").substring(0, Math.min(subject.get("courseName").length(), 18)))
					+ " \u2502 " + String.format("%1$-3s", subject.get("grade")));
		}
	}
	...
	public Integer getGPA() {
		...
		for (HashMap<String, String> subject : this.grades) {
			...
			if (!subject.get("gradePoints").equals("")) {
				...
				gradePointsRevised += (Integer.parseInt(subject.get("gradePoints"))
						/ Integer.parseInt(subject.get("credits")));
				subjects++;
			}
		}
